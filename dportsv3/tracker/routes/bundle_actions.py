...
def register(app, ctx):
    ...
    def api_bundle_accept(
        ...
        try:
            ...
            # R1: the accepted fix nominates the issue for delivery — move it
            # to `resolving` (resolved · awaiting delivery) and record this
            # occurrence as the deliverable. Respects mute/resolved (no-op
            # there); a pre-issue bundle with no issue_key just skips.
            issue_key = row.get("issue_key")
            if issue_key:
                from dportsv3.tracker.routes.issue_actions import (  # noqa: PLC0415
                    mark_issue_resolving,
                )
                mark_issue_resolving(
                    write_conn, issue_key, bundle_id=bundle_id,
                    now=now, actor="operator-accept",
                )

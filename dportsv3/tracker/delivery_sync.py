...
def set_bundle_merged_resolution(
    ...
    # A merge closes the issue only when no confirm build owns it (A2: an
    # issue in `resolving` is resolved by a green build, not by the merge).
...
def resolve_issue_for_bundle(
    ...
    A merged occurrence closes its fingerprinted problem — unless a confirm
    build owns the verdict. Idempotent and defensive: no-op when the bundle
    ...
    if state == "resolved":
        return None
    if state == "resolving":
        # A2: the BUILD is the authority now, not the merge. An issue in
        # `resolving` has a confirm build requested/in flight; letting the
        # merge resolve it here would be exactly the "trust the merge"
        # behaviour this feature removes. Leave it — a green confirm
        # resolves it (A2), a red one reopens it (A3). The operator's

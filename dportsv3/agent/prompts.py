...
5. Build: `dsynth_build(origin)`. This runs dsynth's `test`
   subcommand — the same build `verify` runs before an operator accepts
   your fix, so it compiles the port *and* runs the acceptance-gate
   phases (install, deinstall, check-plist). Inspect `stdout_tail` /
   `stderr_tail`. If `rebuild_ok=true`, you're done; emit the final
   output (below) and stop. If false, return to step 2 with the new
   error info.

   A failure in the gate phases is a real failure: the port built but
   would be refused at acceptance. If it is a pre-existing defect your
   change did not introduce and does not touch, **do not chase it** —
   name it in Patch Log and stop. Reporting a wall you did not build is
   worth more than a budget spent on it.
...
- On `dsynth_build` failure, **call `dsynth_log(origin)`
  immediately**. The real build error is in the
...
- **Knowing when to stop:** once `dsynth_build` returned
  ...
  immediately and stop. Don't keep exploring, and don't re-run the
  build to be sure.
...
Otherwise it must be `false`. It now means the port built *and*
passed the acceptance gate, because that is the only build there is.
Emit **only** `rebuild_ok` — the harness

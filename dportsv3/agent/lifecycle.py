...
class JobState(StrEnum):
    ...
    # A1: build-confirmed resolution. A confirm job lives here from
    # CLAIMED → CONFIRM_START until CONFIRM_GREEN/CONFIRM_RED land at
    # DONE. A red build is a valid *verdict*, not a job failure — the
    # job succeeded if the build ran end-to-end (same discipline as
    # VERIFYING_FIX). CONFIRM_GAVE_UP → DEAD only when the build
    # machinery itself raised.
...
class JobEvent(StrEnum):
    ...
    # A1: confirm-build job lifecycle. CONFIRM_GREEN and CONFIRM_RED are
    # both terminal-DONE (the build produced a verdict); the green/red
    # distinction is the *result*, recorded on the issue, not a job
    # success/failure. CONFIRM_GAVE_UP → DEAD is a machinery failure
    # (env gone, verify-branch checkout failed, apply/build raised).
    CONFIRM_START    = "confirm_start"
    CONFIRM_GREEN    = "confirm_green"
    CONFIRM_RED      = "confirm_red"
    CONFIRM_GAVE_UP  = "confirm_gave_up"
...
TRANSITIONS: dict[tuple[JobState | None, JobEvent], JobState] = {
    ...
    # A1: confirm-build-job happy path (green OR red is a completed
    # verdict → DONE) + machinery failure (→ DEAD).
    (JobState.CLAIMED,     JobEvent.CONFIRM_START):    JobState.CONFIRMING,
    ...
    (JobState.CONFIRMING,  JobEvent.REAP_ORPHAN):      JobState.DEAD,
    ...
    (JobState.CONFIRMING,  JobEvent.ABANDON):          JobState.DEAD,

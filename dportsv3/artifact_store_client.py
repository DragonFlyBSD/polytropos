...
def cmd_bundle_upsert(args):
    payload = {
        ...
        "result": args.result,
        "target": args.target,
        # The tracker build_runs ordinal this failure came from. Empty when
        # tracking is disabled; the store treats that as "no ordinal" and the
        # regression derivation falls back to timestamps.
        "build_run_id": args.build_run_id,
    }
    # Ship the distilled errors text so the store can fingerprint the
    # failure at ingest (issue identity). Best-effort: a missing/unreadable
    # file just means no signature this pass.
    if args.errors_file:
        try:
            with open(args.errors_file, "r", encoding="utf-8", errors="replace") as f:
                payload["errors_text"] = f.read()
        except OSError:
            pass
...
def cmd_job_transition(args):
    detail = {
        ...
        "bundle_id": args.bundle_id,
        "created_ts_utc": args.created_ts_utc,
        "path": args.path,
        "target": args.target,
    ...
    payload = {
        "job_id": args.job_id,
        "event": args.event,
        "actor": args.actor or "hook",
        "detail": detail,
    }
    resp = request_json("POST", f"{args.url}/v1/jobs/transition", payload)
    print(json.dumps(resp, indent=2))
...
def main():
    ...
    p_bundle.add_argument("--target", default="")
    p_bundle.add_argument("--build-run-id", default="")
    p_bundle.add_argument("--errors-file", default="",
                          help="distilled errors.txt to fingerprint at ingest")
    ...
    p_job.add_argument("--event", required=True,
                       help="JobEvent name (e.g. hook_enqueued, claim, "
                            "triage_start, triage_ok, patch_start, "
                            "patch_ok, verify_ok, ...)")
    p_job.add_argument("--actor", default="hook")
    p_job.add_argument("--type", default="")
    p_job.add_argument("--origin", default="")
    p_job.add_argument("--flavor", default="")
    p_job.add_argument("--bundle-dir", default="")
    p_job.add_argument("--created-ts-utc", default="")
    p_job.add_argument("--path", default="")
    p_job.add_argument("--last-error", default="")
    p_job.add_argument("--target", default="")
    p_job.add_argument("--bundle-id", default="",
                       help="bundle this job is attached to; lets the "
                            "store fall back to bundles.target when "
                            "--target is empty")
    p_job.set_defaults(func=cmd_job_transition)

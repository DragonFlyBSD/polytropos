...
# (group_band_by_origin) were removed in the issue-model cutover; the landing
...
@pytest.mark.parametrize("r,v,expected", [
    ...
    ("merged", None, "done"),
    # resolution NULL with no live job: nobody is working it, so it needs a
    # decision. (It used to be None/hidden, which dropped an issue out of the
    # worklist entirely the moment an operator reopened its bundle.) A job
    # that IS in flight still maps to None — see the test below.
    (None, None, "decide"),
])
